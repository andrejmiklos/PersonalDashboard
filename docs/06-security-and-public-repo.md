...
|---|---|---|
| `device` | Tablet | `GET /display/state`, `GET /data/*`, `PATCH /tasks/:sourceId/:taskId` (complete/uncomplete only, in enabled task lists), `POST /display/heartbeat` |
| `admin` | Owner (phone/PC) | Everything, incl. layouts, schedule, settings, accounts/OAuth, tokens, overrides |

Accepted residual risk: a leaked device token can read the same data the tablet shows and toggle the completion
of any task in the task lists chosen for the dashboard (not only the ones a layout shows). It cannot create,
edit or delete tasks, touch other lists, change layouts, settings or accounts, or read other data.

Token format: `dsh_<role>_<43 chars base64url of 32 random bytes>` (prefix helps secret scanners and humans).
Generated by `npm run token:create -- --role admin --label "owner pc" --remote`, which prints the token once and
stores only `SHA-256(token)` in D1. Delivery: the admin token is typed into the hidden prompt of the CLI scripts
(and into the admin app, which keeps it in its browser's `localStorage`; an XSS in the admin would expose it, which is why
the app inserts text only through Preact's escaping, has no `innerHTML` and runs under the strict CSP of §6); a device token reaches the
tablet through a pairing code (§2.1) or, as a fallback, `/display/#t=…` opened once, and stays in Chrome's
`localStorage`.
...
2. **gitleaks** as a pre-commit hook (`.githooks/pre-commit`, enabled by `npm install` through `core.hooksPath`)
   and as a GitHub Action on every PR/push.
...
- **Device token:** pair the display again (`npm run pair -- --remote`, or open `/display/#t=<new>`), then find the
  old token with `npm run token:list -- --remote` and revoke it (`npm run token:revoke -- <id> --remote`).
- **Admin token:** `npm run token:create -- --role admin --label "…" --remote`, use the new one where the old was
  used, then revoke the old one as above.
